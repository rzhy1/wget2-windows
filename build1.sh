...
./configure --build=x86_64-pc-linux-gnu --host=$PREFIX --disable-shared --enable-static --without-lzma --without-zstd --without-bzip2 --without-lzip  --without-libpsl --without-brotlidec --without-gpgme --without-libpcre --without-libpcre2 --without-libidn --without-libid2  --with-winidn --with-ssl=none --enable-threads=windows || exit 1
make -j$(nproc) || exit 1
