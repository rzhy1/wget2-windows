...
build_nghttp2() {
  ...
  local start_time=$(date +%s.%N)
  wget -O- https://github.com/nghttp2/nghttp2/releases/download/v1.64.0/nghttp2-1.65.0.tar.gz | tar xz || exit 1
  cd nghttp2-* || exit 1
...
build_nettle() {
  ...
  wget -O- https://ftp.gnu.org/gnu/nettle/nettle-3.10.1.tar.gz | tar xz || exit 1
  cd nettle-* || exit 1
