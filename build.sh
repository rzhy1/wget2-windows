...
cd brotli || exit 1
CMAKE_SYSTEM_NAME=Windows CMAKE_C_COMPILER=x86_64-w64-mingw32-gcc CMAKE_CXX_COMPILER=x86_64-w64-mingw32-g++ cmake . -DCMAKE_INSTALL_PREFIX=$INSTALLDIR -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_FLAGS="-Ofast -m64 -march=native -mtune=native -flto -funroll-loops -ffunction-sections -fdata-sections -Wl,--gc-sections" -DCMAKE_CXX_FLAGS="-Ofast -m64 -march=native -mtune=native -flto -funroll-loops -ffunction-sections -fdata-sections -Wl,--gc-sections" || exit 1
make install || exit 1
cd .. && rm -rf brotli
...
LDFLAGS="-Wl,--as-needed -L$INSTALLDIR/lib -lzstd -lgnutls -llzma -Wl,-Bstatic $BROTLI_LIBS -Wl,-Bdynamic  -Wl,--no-whole-archive -lwinpthread" ./configure $CONFIGURE_BASE_FLAGS --build=x86_64-pc-linux-gnu --host=$PREFIX --disable-shared --enable-static --with-lzma --with-zstd --without-bzip2 --without-lzip --without-gpgme --with-libiconv-prefix="$INSTALLDIR" --enable-threads=windows || exit 1
make -j$(nproc) || exit 1
