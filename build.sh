...
build_nghttp2() {
  ...
  local start_time=$(date +%s.%N)
  wget -O- https://github.com/nghttp2/nghttp2/releases/download/v1.66.0/nghttp2-1.66.0.tar.gz | tar xz || exit 1
  cd nghttp2-* || exit 1
...
build_wget2() {
  ...
  cd wget2 || exit 1
  sed -i 's|default_gnulib_url=https://git.savannah.gnu.org/git/gnulib.git|default_gnulib_url=https://github.com/coreutils/gnulib.git|' bootstrap
