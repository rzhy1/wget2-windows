...
./configure --build=x86_64-pc-linux-gnu --host=$PREFIX --with-libiconv-prefix="$INSTALLDIR" --with-ssl=gnutls --disable-shared --enable-static --without-lzma --without-zstd --without-bzip2 --without-lzip  --without-libpsl --without-brotlidec --with-ssl=wolfssl --without-gpgme --enable-threads=windows || exit 1
make -j$(nproc) || exit 1
