...
build_nghttp2() {
  ...
  local start_time=$(date +%s.%N)
  wget -O- https://github.com/nghttp2/nghttp2/releases/download/v1.68.1/nghttp2-1.68.1.tar.gz | tar xz || exit 1
  cd nghttp2-* || exit 1
...
build_gnutls() {
  ...
  if ! wget -q -O- https://www.linuxfromscratch.org/patches/blfs/svn/gnutls-3.8.12-nettle4_fixes-1.patch | patch -Np1; then
    echo "Patch failed!" >&2
    exit 1
  fi
